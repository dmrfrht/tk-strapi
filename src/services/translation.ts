...
export default ({ strapi }: { strapi: Core.Strapi }) => {
  ...
  const getOpenAIClient = () => {
    ...
    strapi.log.info(`OpenAI Model: ${process.env.OPENAI_MODEL || 'gpt-4o-mini'}`);
  ...
  const translateWithLibreTranslate = async (
    ...
    const sourceLang = localeToCode[sourceLocale] || sourceLocale.split('-')[0].toLowerCase();
    const targetLang = localeToCode[targetLocale] || targetLocale.split('-')[0].toLowerCase();

    try {
      strapi.log.info(`LibreTranslate: Translating from ${sourceLang} to ${targetLang}, text length: ${text.length}`);
      ...
      strapi.log.info(`LibreTranslate: Request body:`, JSON.stringify(requestBody).substring(0, 200));
      ...
      strapi.log.info(`LibreTranslate: Response status: ${response.status}, body: ${responseText.substring(0, 200)}`);
      ...
      strapi.log.info(`LibreTranslate: Translation successful, length: ${translatedText.length}`);
  ...
  const translateWithMyMemory = async (
    ...
    // MyMemory free tier has limits: max 500 words per request
    // Split long text into chunks if needed
    const maxLength = 500;
    const words = text.split(/\s+/);
    
    if (words.length > maxLength) {
      strapi.log.warn(`Text too long (${words.length} words), splitting into chunks`);
      const chunks: string[] = [];
      ...
      const translatedChunks = await Promise.all(
        chunks.map(chunk => translateWithMyMemory(chunk, sourceLocale, targetLocale))
      );
    ...
    try {
      ...
      if (data.responseStatus !== 200) {
        const errorMessage = typeof data.responseData === 'string' 
          ? data.responseData 
          : (data.responseData?.message || JSON.stringify(data.responseData) || 'Unknown error');
        
        // Handle rate limiting
        if (data.responseStatus === 429 || errorMessage.includes('quota') || errorMessage.includes('limit')) {
          strapi.log.warn('MyMemory rate limit reached, waiting 1 second before retry...');
          await new Promise(resolve => setTimeout(resolve, 1000));
          // Retry once
          return translateWithMyMemory(text, sourceLocale, targetLocale);
        }
        
        throw new Error(`MyMemory translation error (${data.responseStatus}): ${errorMessage}`);
      }
      ...
      if (errorMessage.includes('429') || errorMessage.includes('quota') || errorMessage.includes('limit')) {
        throw new Error(`MyMemory rate limit exceeded. Please wait a moment and try again, or use a different translation provider.`);
      }
  ...
  async translateText(
    ...
    strapi.log.info(`Translating text from ${sourceLocale} to ${targetLocale} using ${TRANSLATION_PROVIDER}, length: ${text.length}`);
    ...
    switch (TRANSLATION_PROVIDER.toLowerCase()) {
      case 'libretranslate':
        ...
        if (process.env.LIBRETRANSLATE_API_URL) {
          try {
            return await translateWithLibreTranslate(text, sourceLocale, targetLocale);
          } catch (error: any) {
            strapi.log.warn('LibreTranslate failed, falling back to MyMemory:', error.message);
            // Fallback to MyMemory if LibreTranslate fails
            return translateWithMyMemory(text, sourceLocale, targetLocale);
          }
        } else {
          strapi.log.warn('LibreTranslate API URL not configured, using MyMemory instead');
          return translateWithMyMemory(text, sourceLocale, targetLocale);
        }
      ...
      default:
        ...
        const sourceLanguage = localeToLanguage[sourceLocale] || sourceLocale.split('-')[0];
        const targetLanguage = localeToLanguage[targetLocale] || targetLocale.split('-')[0];
        ...
        try {
          ...
          strapi.log.info(`Translation successful, response length: ${translatedText.length}`);
          return translatedText;
        } catch (error: any) {
          ...
          if (error.response) {
            ...
            if (error.response.status === 429) {
              const errorMessage = error.response.data?.error?.message || 'Quota exceeded';
              throw new Error(`OpenAI API quota exceeded: ${errorMessage}. Please check your billing at https://platform.openai.com/account/billing`);
            }
  ...
  async translateContent(
    ...
    strapi.log.info(`Looking for entry: ${contentType} #${entryId} in locale ${sourceLocale}`);
    
    try {
      // If entryId is a string, it's likely a documentId - use documents API
      if (typeof entryId === 'string') {
        try {
          strapi.log.info(`Trying to find via documents API with documentId: ${entryId}`);
          sourceEntry = await strapi.documents(contentType as any).findOne({
            documentId: entryId,
            locale: sourceLocale,
          });
          
          strapi.log.info(`Documents API result:`, sourceEntry ? 'Found' : 'Not found');
          ...
          strapi.log.warn('Error finding entry via documents API:', docError.message);
          ...
          try {
            ...
            strapi.log.warn('Error finding entry via entityService:', entityError.message);
          }
        }
      } else {
        // If entryId is a number, use entityService directly
        strapi.log.info(`Trying to find via entityService with id: ${entryId}`);
        sourceEntry = await strapi.entityService.findOne(
          contentType as any,
          entryId,
          {
      ...
      if (!sourceEntry) {
        throw new Error(`Entry not found: ${contentType} #${entryId} in locale ${sourceLocale}`);
      }
      
      strapi.log.info(`Successfully found entry: ${sourceEntry.id || sourceEntry.documentId}`);
    } catch (error: any) {
      strapi.log.error('Error finding source entry:', error);
      throw new Error(`Entry not found: ${contentType} #${entryId} in locale ${sourceLocale}. Error: ${error.message}`);
    }
    ...
    for (const [fieldName, fieldConfig] of Object.entries(contentTypeSchema.attributes)) {
      const field = fieldConfig as any;
      
      // Skip system fields and unique fields that shouldn't be copied
      if (['id', 'createdAt', 'updatedAt', 'publishedAt', 'createdBy', 'updatedBy', 'locale', 'localizations'].includes(fieldName)) {
        continue;
      }
      ...
      if (field.unique === true && fieldName === 'legacyId') {
        strapi.log.info(`Skipping unique field: ${fieldName} (will be auto-generated or left empty)`);
        continue;
      }
    ...
    try {
      ...
      if (sourceEntry.documentId) {
        try {
          existingTranslation = await strapi.documents(contentType as any).findOne({
            documentId: sourceEntry.documentId,
            locale: targetLocale,
          });
        } catch (findError: any) {
          strapi.log.info('No existing translation found, will create new one');
        }
      }
      
      if (existingTranslation && existingTranslation.id) {
        // Update existing translation
        strapi.log.info(`Updating existing translation with id: ${existingTranslation.id}, locale: ${targetLocale}`);
        ...
        strapi.log.info(`Translation updated successfully. ID: ${translatedEntry.id}, Document ID: ${translatedEntry.documentId || 'N/A'}, Locale: ${translatedEntry.locale || targetLocale}`);
      } else if (sourceEntry.documentId) {
        // Create new translation using clone to properly link translations
        strapi.log.info(`Creating new translation using clone API for locale: ${targetLocale}`);
        
        try {
          // Use clone to create translation linked to source (same documentId)
          const cloneResult: any = await strapi.documents(contentType as any).clone({
            documentId: sourceEntry.documentId,
            locale: targetLocale,
          });
          
          strapi.log.info(`Clone API result:`, JSON.stringify(cloneResult, null, 2));
          ...
          if (cloneResult && cloneResult.id) {
            ...
            strapi.log.info(`Clone result doesn't contain ID, finding cloned entry manually`);
            try {
              clonedEntry = await strapi.documents(contentType as any).findOne({
                documentId: sourceEntry.documentId,
                locale: targetLocale,
              ...
              if (!clonedEntry || !clonedEntry.id) {
                ...
                strapi.log.warn(`No existing translation found for documentId ${sourceEntry.documentId} in locale ${targetLocale}`);
                throw new Error('Clone operation did not create translation entry');
              }
            } catch (findError: any) {
              strapi.log.warn(`Could not find cloned entry: ${findError.message}`);
              throw new Error(`Clone operation failed: ${findError.message}`);
            }
          }
          
          if (!clonedEntry || !clonedEntry.id) {
            throw new Error('Could not find cloned entry after clone operation');
          }
          
          strapi.log.info(`Found cloned entry. ID: ${clonedEntry.id}, Document ID: ${clonedEntry.documentId}, Locale: ${clonedEntry.locale || targetLocale}`);
          ...
          strapi.log.info(`Translation updated with translated content. ID: ${translatedEntry.id}, Document ID: ${translatedEntry.documentId || 'N/A'}, Locale: ${translatedEntry.locale || targetLocale}`);
          ...
          strapi.log.warn(`Clone API not available or failed (this is OK): ${errorMessage}`);
          ...
          if (existingTranslation && existingTranslation.id) {
            // Update existing translation
            strapi.log.info(`Updating existing translation with id: ${existingTranslation.id}`);
            translatedEntry = await strapi.entityService.update(
              contentType as any,
              existingTranslation.id,
              {
            ...
            strapi.log.info(`Translation updated successfully. ID: ${translatedEntry.id}, Locale: ${translatedEntry.locale || targetLocale}`);
          } else {
            // Create new translation with entityService
            strapi.log.info('Creating new translation using entityService.create');
            translatedEntry = await strapi.entityService.create(contentType as any, {
              data: translatedData,
              locale: targetLocale,
            });
            
            strapi.log.info(`Translation created successfully. ID: ${translatedEntry.id}, Document ID: ${translatedEntry.documentId || 'N/A'}, Locale: ${translatedEntry.locale || targetLocale}`);
        ...
        strapi.log.info(`Creating new translation entry without documentId for locale: ${targetLocale}`);
        translatedEntry = await strapi.entityService.create(contentType as any, {
          data: translatedData,
          locale: targetLocale,
        });
        strapi.log.info(`Translation created. ID: ${translatedEntry.id}, Document ID: ${translatedEntry.documentId || 'N/A'}, Locale: ${translatedEntry.locale || targetLocale}`);
      }
      ...
      if (translatedEntry.documentId) {
        try {
          ...
          strapi.log.warn('Could not auto-publish translated entry (this is OK, user can publish manually):', publishError.message);
        }
        ...
        try {
          ...
          const contentTypeSchema = strapi.contentTypes[contentType];
          if (contentTypeSchema && contentTypeSchema.options?.draftAndPublish) {
          ...
          strapi.log.warn('Could not publish translated entry:', publishError.message);
        }
      }
    } catch (error: any) {
